...
async function getUserBreakdown(supabase) {
  const { data, error } = await supabase
    .from(TABLES.RESULTS)
    .select('user_id, cii_score, completed_at, employee_name');

  if (error) throw error;

  const userMap = {};
  data.forEach(row => {
    const key = row.user_id || row.employee_name || 'Unknown';
    if (!userMap[key]) {
      userMap[key] = { userId: key, name: row.employee_name || 'Unknown', attempts: 0, scoreSum: 0, lastActivity: null };
    }
    userMap[key].attempts++;
    userMap[key].scoreSum += (row.cii_score || 0);
    if (row.completed_at) {
      const d = new Date(row.completed_at);
      if (!userMap[key].lastActivity || d > new Date(userMap[key].lastActivity)) {
        userMap[key].lastActivity = row.completed_at;
      }
    }
  });
